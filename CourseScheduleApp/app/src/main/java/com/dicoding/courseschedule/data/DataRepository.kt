...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType): LiveData<Course?> {
        val getNearestQuery: SupportSQLiteQuery = nearestQuery(type = queryType)
        return dao.getNearestSchedule(getNearestQuery)
    }

    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val sortTypeQuery: SimpleSQLiteQuery = QueryUtil.sortedQuery(sortType)
    ...
    fun getTodaySchedule(): List<Course> {
        var getDay = GregorianCalendar().get(Calendar.DAY_OF_WEEK) - 1
        if (getDay == 0)
            getDay = 7
